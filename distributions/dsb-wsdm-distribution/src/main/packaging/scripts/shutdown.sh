...
usage()
{
	echo "The shutdown.sh script is used to shutdown Petals."
...
then
  ...
  echo "This file is required to shutdown PEtALS."
  exit 1
fi
...
if [ "$Confirmed" = "false" ]
then
	echo "Are you sure to remove this PEtALS container from your PEtALS network (Y/n) ?"
	echo "    - all services assembly of this container will be undeployed,"
	echo "    - all components of this container will be uninstalled,"
	echo "    - this PEtALS container will be removed from the topology."
	read ANSWER
	if [ "$ANSWER" = "Y" ]
	then
		Confirmed=true
