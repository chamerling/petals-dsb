...
import org.ow2.petals.binding.soap.listener.incoming.servlet.ListTopicsServlet;
import org.ow2.petals.binding.soap.listener.incoming.servlet.SoapServlet;
import org.ow2.petals.binding.soap.listener.incoming.servlet.SubscriptionServiceServlet;
...
/**
 * Create a Jetty server that will be the axis2 transportlistener. This Jetty
...
public class SoapServer implements TransportListener {
     ...
     * 
     */
    protected WsnIsolationLayer isolation;
     ...
     * @throws AxisFault
     */
    public SoapServer(final SoapServerConfig config,
            final SoapComponentContext componentContext, final Logger logger) throws AxisFault {
        this.config = config;
        this.logger = logger;
        ...
        final Context wsnContext = new Context(contexts, "/" + this.config.getWsnContext(),
                Context.SESSIONS);
        final WsnManager manager = componentContext.getWsnManager();
        if (manager.getProducerEPR() == null) {
            manager.setProducerEPR(new org.ow2.petals.ws.addressing.EndpointReference(URI
                    .create(this.config.getWSNProducerURL())));
        }
        this.isolation = new WsnIsolationLayer(manager);

        final ServletHolder subscribeServlet = new ServletHolder(new SubscriptionServiceServlet(
                this.isolation, logger));
        subscribeServlet.setName("SubscribeServlet");
        wsnContext.addServlet(subscribeServlet, "/" + this.config.getWsnProducerMapping());

        // add the topic information servlet
        final ServletHolder topicServlet = new ServletHolder(new ListTopicsServlet(manager,
                this.config));
        topicServlet.setName("TopicsListServlet");
        topicServlet.setInitOrder(1);
        axisContext.addServlet(topicServlet, "/" + this.config.getServicesMapping() + "/"
     ...
     * (non-Javadoc)
     * 
     * @see org.apache.axis2.transport.TransportListener#getEPRsForService(java.lang.String,
     *      java.lang.String)
     */
     ...
     * (non-Javadoc)
     * 
     * @see org.apache.axis2.transport.TransportListener#init(org.apache.axis2.context.ConfigurationContext,
