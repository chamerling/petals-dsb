...
import org.ow2.petals.esb.api.EndpointQuery;
...
public class JAXBEndpointQueryBuilder {
    ...
    static {
        try {
            final JAXBContext jaxbContext = JAXBContext
                    .newInstance(new Class[] { EndpointQuery.class });
     ...
     * @param os
     */
    public static final void marshall(EndpointQuery endpointQuery, OutputStream os)
            throws MessageExchangeException {
        if ((endpointQuery != null) && (os != null)) {
            try {
                JAXBElement<EndpointQuery> element = new JAXBElement<EndpointQuery>(new QName(
                        "org.ow2.petals.esb.api", "EndpointQuery"), EndpointQuery.class, null,
                        endpointQuery);
     ...
     * @throws MessageExchangeException
     */
    public static final EndpointQuery unmarshall(InputStream is) throws MessageExchangeException {
        EndpointQuery m = null;
        if (is != null) {
            try {
                final JAXBElement<EndpointQuery> root;
                // The default Xerces unmarshaller is not thread safe
                synchronized (unmarshaller) {
                    root = unmarshaller.unmarshal(new StreamSource(is), EndpointQuery.class);
                }
                m = root.getValue();
            } catch (Exception e) {
                throw new MessageExchangeException(
